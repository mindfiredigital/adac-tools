...
export class AWSPricing {
  ...
  getFargateVcpuPrice(pricingModel?: PricingModel): number {
    return this.getTieredPrice(data.ecs['fargate_vcpu'], pricingModel);
  }

  getFargateMemoryPrice(pricingModel?: PricingModel): number {
    return this.getTieredPrice(data.ecs['fargate_memory'], pricingModel);
  }
