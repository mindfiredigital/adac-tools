...
describe('ComputeCalculator', () => {
  ...
  it('should scale linearly with instance count for t3.micro', () => {
    ...
    // 3 instances should cost ~3x a single instance
    expect(cost3).toBeCloseTo(cost1 * 3, 1); // precision relaxed
  });
  ...
  it('should sum cost across multiple EC2 service entries', () => {
    ...
    expect(costMulti).toBeCloseTo(cost1 * 3, 1); // precision relaxed
  });
});
