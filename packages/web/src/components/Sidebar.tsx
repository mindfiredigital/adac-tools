...
export const Sidebar = () => {
    ...
    const onDragStart = (event: React.DragEvent, nodeType: string, iconPath: string, label: string) => {
    ...
    return (
        <aside className="w-80 h-full bg-[#1e1e1e] border-r border-[#333] flex flex-col overflow-hidden text-neutral-300 font-sans shadow-xl z-10">
            <div className="p-5 border-b border-[#333] backdrop-blur-md bg-[#1e1e1e]/90 sticky top-0 z-20">
                ...
                <p className="text-xs text-gray-500 mt-1 font-medium">Drag components to the board</p>
            </div>
                ...
                {categories.map((cat) => (
                    <div key={cat.category} className="group">
                        ...
                        <div className="grid grid-cols-3 gap-2">
                            {cat.icons.map((icon) => {
                                const label = icon.name.replace(/^Arch_/, '').replace(/_48\.png$/, '').replace(/_/g, ' ');
                                return (
                                    <div 
                                        key={icon.name}
                                        ...
                                        onDragStart={(event) => onDragStart(event, 'customNode', icon.path, label)}
                                        draggable
                                        title={label}
                                    >
                                        <img src={icon.path} alt={label} className="w-8 h-8 mb-1 pointer-events-none object-contain" />
