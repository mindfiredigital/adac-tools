...
export const Uploader = ({ onBack }: UploaderProps) => {
  ...
  return (
    <div className="flex flex-col h-full bg-[#121212] p-8 overflow-y-auto">
      <button
        onClick={onBack}
        className="self-start mb-8 text-gray-400 hover:text-white flex items-center gap-2"
      >
        &larr; Back to Home
      </button>

      <div
        className={`mx-auto w-full my-auto transition-all duration-500 ${svgContent ? 'max-w-[95vw]' : 'max-w-3xl'}`}
      >
        <div
          className={`grid gap-8 ${svgContent ? 'grid-cols-1 lg:grid-cols-2 h-[80vh]' : 'grid-cols-1'}`}
        >
          {/* Upload Section */}
          <div
            className={`bg-[#1e1e1e] p-8 rounded-xl border border-[#333] shadow-2xl ${svgContent ? 'h-full flex flex-col' : ''}`}
          >
            ...
            <div
              className={`flex flex-col gap-6 ${svgContent ? 'flex-1 justify-center' : ''}`}
            >
