...
Based on the [feature specifications](https://github.com/mindfiredigital/adac-tools/issues/15), the Diagram package's core responsibility is to act as a robust rendering engine that takes a computed layout result and generates high-quality visual assets.

### Core Objectives

- **Visual Generation**: Implementing complete SVG and PNG generation that precisely matches the calculated layout from the layout engine.
- **Icon Mapping**: Intelligently mapping ADAC Resource Types to their respective provider icons (AWS, Azure, GCP).
- **Theming System**: Applying comprehensive theming (light, dark, custom) to the rendered architecture diagrams.

### Internal API Exports

- `renderDiagram(layout: LayoutResult, format: 'svg'|'png'): Buffer|string`

### Key Dependencies

- `@mindfiredigital/adac-layout-engine`
- `@mindfiredigital/adac-core-schema`
