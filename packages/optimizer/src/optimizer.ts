...
export class OptimizerEngine {
  ...
  public analyze(config: AdacConfig): OptimizationResult {
    ...
    if (this.options.enableCostRules) raw.push(...runCostRules(config));
    if (this.options.enableSecurityRules) raw.push(...runSecurityRules(config));
    ...
    for (const rec of filtered) {
      for (const resourceId of rec.affectedResources) {
        if (!byService[resourceId]) byService[resourceId] = [];
