...
<body>
  ...
  for (const file of distinctFiles) {
    const baseName = path
      .basename(file, path.extname(file))
      .replace('.adac', '');
    const elkReport = reports.find(
      (r) => r.file === file && r.engine === 'elk'
    );
    const dagreReport = reports.find(
      (r) => r.file === file && r.engine === 'dagre'
    );
